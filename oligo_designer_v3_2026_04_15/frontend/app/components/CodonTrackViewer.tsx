...
export default function CodonTrackViewer({ codons, codonTable, warnings, originalWarnings, onDnaChange, onUndo, canUndo, gblockSegments }: Props) {
  ...
  const hasGblocks = (gblockSegments ?? []).length > 0;
  const gblockTrackH = hasGblocks ? GBLOCK_TRACK_H + GBLOCK_GAP : 0;
  const contentStartY = RULER_H + warningTrackH;
  const codonBlockH = AA_ROW_H + ROW_GAP + CODON_ROW_H + ROW_GAP + VALUE_ROW_H;
  const gblockTrackY = contentStartY + codonBlockH + GBLOCK_GAP;
  const totalH = contentStartY + codonBlockH + gblockTrackH + BOTTOM_PAD;
      ...
      <div className="flex">
        ...
        <div style={{ width: LEFT_MARGIN, flexShrink: 0 }}>
          <svg width={LEFT_MARGIN} height={totalH} className="select-none">
            ...
            {hasGblocks && (
              <text x={4} y={gblockTrackY + GBLOCK_TRACK_H / 2 + 3} fontSize={9} fill={GBLOCK_COLOR} fontWeight="600">gBlock</text>
            )}
        ...
        >
          ...
          {/* gBlock regions track — below the codons */}
          {(gblockSegments ?? []).map((gb, i) => {
            const x1 = bp2x(gb.start_bp);
            const x2 = bp2x(gb.end_bp);
            const y = gblockTrackY;
            return (
              <g key={`gblock-${i}`}>
                <rect
                  x={x1}
                ...
                {cellW >= 8 && (
                  <text
                    x={x1 + 4}
                    y={y + GBLOCK_TRACK_H - 5}
                    fontSize={9}
                    fill={GBLOCK_COLOR}
                    fontWeight="600"
                  >
                    {gb.label || "gBlock"} ({gb.len_bp} bp)
                  </text>
                )}
              </g>
            );
