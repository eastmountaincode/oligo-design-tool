...
export default function CodonTrackViewer({ codons, codonTable, warnings, originalWarnings, onDnaChange }: Props) {
  ...
  useEffect(() => {
    ...
    const x = LEFT_MARGIN + selectedIndex * cW;
  ...
  const handleZoom = useCallback((newZoom: number) => {
    ...
    const centerCodonIndex = (viewportCenter - LEFT_MARGIN) / oldCellW;
    setZoom(newZoom);
    requestAnimationFrame(() => {
      const newCellW = CODON_W * newZoom;
      sc.scrollLeft = LEFT_MARGIN + centerCodonIndex * newCellW - sc.clientWidth / 2;
    });
  }, [zoom]);
  ...
  const totalW = LEFT_MARGIN + codons.length * cellW;
