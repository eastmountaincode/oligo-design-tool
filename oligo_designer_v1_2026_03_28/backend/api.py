...
class CodonOptRequest(BaseModel):
    ...
    avoid_homopolymers: int = Field(5, ge=3, le=8, description="Avoid runs of N+ identical bases")
...
def codon_optimize(req: CodonOptRequest):
    ...
    for base in "ATCG":
        constraints.append(AvoidPattern(base * req.avoid_homopolymers))
    ...
    # Build raw /1000 table (used for both per-codon detail and table display)
    ...
    # Run k-mer uniqueness check on the optimized DNA (detection only, not enforced)
    kmer_problem = DnaOptimizationProblem(
        sequence=opt_dna,
        constraints=[UniquifyAllKmers(req.uniquify_kmers)],
    )

    # Constraint results (include both optimization constraints + k-mer check)
    summary = []
    ...
    all_pass = problem.all_constraints_pass() and kmer_problem.all_constraints_pass()
...
class CheckConstraintsRequest(BaseModel):
    ...
    avoid_homopolymers: int = Field(5, ge=3, le=8)
    gc_min: float = Field(0.25, ge=0.0, le=1.0)
    gc_max: float = Field(0.75, ge=0.0, le=1.0)
    gc_window: int = Field(50, ge=20, le=200)
...
def _scan_warnings(dna: str, gc_min: float = 0.25, gc_max: float = 0.75,
    ...
    # Homopolymer runs of 4+ (informational, even if constraint is set to 5+)
    for base in "ATCG":
        i = 0
        while i < len(s):
            if s[i] == base:
                ...
                if run_len >= 4:
                    warnings.append({
                        "kind": "homopolymer",
                        "message": f"{run_len}x {base} at position {i}-{j}",
                        "start": i,
                        "end": j,
                    })
                i = j
...
def check_constraints(req: CheckConstraintsRequest):
    ...
    constraints.append(UniquifyAllKmers(req.uniquify_kmers))
    for pat in req.avoid_patterns:
        constraints.append(AvoidPattern(pat))
